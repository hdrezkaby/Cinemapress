    ...
    function(group, group_index, callback) {
      ...
      async.eachOfLimit(
        ...
        function(task, index, callback) {
          ...
          var req_url = (parse[0] || '').replace(/POST>/i, '').trim();
          req_url =
            req_url && /^(http|\/\/)/i.test(req_url)
              ? req_url
              : config.protocol + config.subdomain + config.domain + req_url;
          request(
            {
              url: req_url,
              method: /POST>/i.test(parse[0] || '') ? 'POST' : 'GET',
              timeout: 5000
            },
            function(error, response, body) {
              if (active.num <= 0) {
              ...
              try {
                ...
                var old_scraper = fs.existsSync(file_scraper)
                  ? tryParseJSON(fs.readFileSync(file_scraper, 'utf8'))
                  : {};
                var key =
                  parse[1] && parse[1].split('<>') && parse[1].split('<>')[1]
                    ? parse[1].split('<>')[1].trim()
                    : 'id';
                var movie_ids = movies.map(function(movie) {
                  return movie.id;
                });
                if (
                  old_scraper &&
                  old_scraper.date &&
                  new Date(old_scraper.date).getHours() ===
                    new Date().getHours()
                ) {
                  old_scraper.movies.forEach(function(movie) {
                    if (movie_ids.indexOf(movie) === -1) {
...
if (
  ...
  async.eachOfLimit(
    ...
    function(task, index, callback) {
      ...
      var req_method = /^POST>/i.test(parse[0].split('<>')[0].trim());
      var req_url = parse[0]
        .split('<>')[0]
        .trim()
        .replace('[yesterday]', yyyy_mm_dd(date))
        .replace('POST>', '');
      req_url =
        req_url && /^(http|\/\/)/i.test(req_url)
          ? req_url
        ...
        function(error, response, body) {
          ...
          Object.keys(ids)
            ...
            .forEach(function(id) {
              ...
              var translates = Object.assign({}, t[0], {
                translate_url: (t && t[0] && t[0].translate) || '',
                translate: t
                  ? t
          ...
          async.eachOfLimit(
            ...
            function() {
              ...
              file_episodes.forEach(function(file_episode) {
                if (episodes.length >= modules.episode.data.index.max) {
                  return;
                }
                var set_episode_id = true;
                episodes.forEach(function(episode) {
                  if (file_episode.id === episode.id) {
                    set_episode_id = false;
                  }
                });
                if (set_episode_id) {
                  episodes.push(file_episode);
                } else {
                  var set_episode = true;
                  episodes.forEach(function(episode) {
                    if (
                      file_episode.season < episode.season ||
                      (file_episode.season === episode.season &&
                        file_episode.episode <= episode.episode)
                    ) {
                      set_episode = false;
                    }
                  });
...
if (
  ...
  CP_get.publishIds(function(err, ids) {
    ...
    if (!ids) {
      ...
      if (ids.stop_id > 10000000) {
        ids.stop_id = 10000000;
      }
      if (ids.start_id === 298 && ids.stop_id === 10000000) {
                  ...
                  function(err, structures_CP_XMLPIPE2) {
                    ...
                    CP_get.movies(
                      ...
                      function(err, movies_CP_RT) {
                        ...
                        const prompt = `Перепиши следующее описание фильма, чтобы оно звучало уникально, естественно и не теряло оригинального смысла:\n\n"${movie_CP_XMLPIPE2.description}"\n\nНовое описание:`;
                        const OPENAI_API_KEY = modules.rewrite.data.token;
                        axios.post('https://api.openai.com/v1/chat/completions', {
                          model: 'gpt-4',
                          messages: [
                            { role: 'system', content: 'Ты профессиональный копирайтер по созданию уникального контента.' },
                        ...
                        .then(function(response) {
                          ...
                          const custom = movie_CP_XMLPIPE2.custom ? JSON.parse(movie_CP_XMLPIPE2.custom) : {};
                          custom.unique = true;
                          movie_CP_XMLPIPE2.custom = JSON.stringify(custom);
                          movie_CP_XMLPIPE2.description = rewritten;
                        ...
                        .catch(function(err) {
                          console.error('[ChatGPT rewrite error]', err.response.data || err.message);
                          callback();
                        });
                      }
                    );
              ...
              render.urls[render.urls.length] = {
                ...
                lastmod:
                  sitemap_movies[i].custom && sitemap_movies[i].custom.lastmod
                    ...
                    : '',
                image: sitemap_movies[i].poster
                  ? /^(http|\/\/)/i.test(sitemap_movies[i].poster)
